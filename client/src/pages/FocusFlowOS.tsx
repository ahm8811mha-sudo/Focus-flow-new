...
const modules = [
  ...
  { title: 'مركز النظام', icon: '⚙️', href: '/system', desc: 'OpenAI، Google Drive، Calendar، Gmail Drafts، Google Sheets، والنسخ السحابي.' },
...
export default function FocusFlowOS() {
  ...
  useEffect(() => {
    fetch('/api/agents/status')
      .then((res) => res.json())
      .then((data) => setOpenaiReady(Boolean(data?.openaiConfigured)))
  ...
  return (
    <main className="os-shell" dir="rtl">
      ...
      <header className="hero">
        ...
        <span className="eyebrow">Personal AI Execution OS</span>
        <h1>Focus Flow OS</h1>
        <p>النظام الموحد للوكلاء والمهام والتقويم والنسخ السحابي. الوكلاء هم القلب، والمهام والخدمات هي أدوات التنفيذ.</p>
        <div className="hero-actions">
          <a className="primary" href="/agents">ابدأ من الوكلاء التنفيذيين</a>
          <a className="secondary" href="/system">مركز النظام والخدمات</a>
        </div>
      </header>

      <section className="status-grid">
        <div className="status-card"><span>OpenAI</span><strong>{openaiReady ? 'متصل' : 'غير مفعّل'}</strong><small>{openaiReady ? 'الوكلاء جاهزون للقرارات' : 'أضف OPENAI_API_KEY في Vercel'}</small></div>
      ...
      <section className="main-grid">
        <article className="panel command-panel">
          <div className="section-title"><h2>أمر سريع للقائد العام</h2><a href="/agents">فتح مركز الأوامر</a></div>
          <p>استخدم هذا المسار عندما تريد قرار وتنفيذ: إنشاء مهمة، ملاحظة، مسودة Gmail، Google Sheet، أو حدث تقويم.</p>
          <div className="prompt-box">رتب يومي، قرر أهم 3 إجراءات، ونفّذها داخل النظام.</div>
          <a className="primary wide" href="/agents">تشغيل الوكلاء</a>
        </article>

        <article className="panel">
          <div className="section-title"><h2>أهم المهام المفتوحة</h2><a href="/focus">إدارة المهام</a></div>
          <div className="task-list">
            ...
            {!topTasks.length && <small>لا توجد مهام مفتوحة. ابدأ من زر المهام.</small>}
          </div>
        </article>
      </section>
