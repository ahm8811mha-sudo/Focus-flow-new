...
const SESSIONS_KEY = 'focus-flow-agent-sessions';
...
const agents: Agent[] = [
  ...
  { id: 'assistant', name: 'السكرتير', icon: '🗂️', promise: 'يبحث ويحصر وينظم الاتصالات والمتابعات.', examples: ['احصر عيادات أطفال الأنابيب في الرياض واتصل عليهم من 4 إلى 5', 'ليش ما ظهر البريد في الجدول؟'] },
  { id: 'style', name: 'الستايل', icon: '👔', promise: 'يحلل صورة اللبس ويجهز قائمة مشتريات ومهام مقارنة.', examples: ['حلل الصورة وابحث عن مشابه', 'عدّل القائمة وخليها مرتبة من الأرخص للأغلى'] },
  { id: 'finance', name: 'المال', icon: '💳', promise: 'ينشئ خطة سداد وجداول وتنبيهات مالية.', examples: ['رتب ديوني وخطة السداد', 'اشرح لي ليش رتبتها بهذا الشكل'] },
  { id: 'message', name: 'الرسائل', icon: '✉️', promise: 'ينشئ مسودات Gmail وتذكيرات متابعة.', examples: ['اكتب إيميل رسمي واحفظ متابعة', 'عدّل الصياغة وخليها أكثر رسمية'] },
  { id: 'calendar', name: 'التقويم', icon: '📅', promise: 'ينشئ مواعيد وتنبيهات وبلوكات عمل.', examples: ['جدول لي متابعة غدًا 9', 'وين ظهر الموعد؟'] },
  { id: 'excel', name: 'Excel', icon: '📊', promise: 'ينشئ جداول متابعة ومهام تحديث.', examples: ['جهز جدول متابعة', 'عدّل الجدول وأضف أعمدة الحالة والمسؤول والملاحظات'] },
...
function normalizeType(type: string): ActionType { if (type === 'calendar_event' || type === 'google_calendar') return 'calendar_file'; if (type === 'google_sheet' || type === 'sheet' || type === 'shopping_table') return 'csv_file'; if (type === 'email_draft' || type === 'gmail_draft') return 'message_draft'; return ['create_task', 'calendar_file', 'csv_file', 'message_draft', 'create_note'].includes(type) ? type as ActionType : 'create_task'; }
...
export default function ExecutionAgentsPage() {
  ...
  async function executeAction(action: Action) {
    ...
    if (safe.type === 'csv_file') { try { await createGoogleSheet({ title: safe.title, columns: safe.payload?.columns || ['العنوان', 'الوصف'], rows: safe.payload?.rows || [[safe.title, safe.description]] }); } catch { await memory.saveNote({ title: `جدول محفوظ: ${safe.title}`, body: tableText(safe), tags: ['table', 'agent', agent.id], pinned: true }); } }
  }

  async function runActions(items: Action[], session: Session, summary: string, goal: string) {
    const done: Action[] = [];
    const failures: string[] = [];
    for (const action of items.map(ensureRows)) { try { await executeAction(action); done.push({ ...action, executed: true }); } catch { done.push({ ...action, failed: true }); failures.push(action.title); } }
    const reply = `${summary}\n\nتم تنفيذ/تحديث ${done.filter((a) => a.executed).length} إجراء. ${failures.length ? `فشل: ${failures.join(', ')}` : 'لم يتم تنزيل ملفات تلقائيًا.'}`;
    ...
    addExecutionLog({ agentName: agent.name, goal, status: failures.length ? 'needs_user' : 'done', summary, results: done.filter((a) => a.executed).map((a) => `${a.title} (${a.type})`), failures, nextSteps: failures.length ? ['راجع الإجراء الفاشل أو أعد المحاولة من نفس المحادثة.'] : ['تابع من نفس المحادثة إذا أردت تعديل النتيجة.'] });
  }
  ...
  async function send() {
    ...
    try {
      ...
      const instruction = `${conversation}${previous}\n\nنفّذ فقط إذا كان الطلب الحالي فيه أمر واضح. إذا كان المستخدم يعدّل نتيجة سابقة فعدّلها. لا تنشئ جدولًا فارغًا ولا تنزّل ملفات.`;
