...
function titleOf(text: string) {
  return text.trim().replace(/\s+/g, ' ').slice(0, 46) || 'محادثة جديدة';
}
...
function saveSessions(items: Session[]) {
  localStorage.setItem(SESSIONS_KEY, JSON.stringify(asArray(items)));
}

function isDirectoryRequest(text: string) {
  return /احصر|ابحث|أرقام|ارقام|هواتف|هاتف|عيادات|شركات|مطاعم|مراكز|مستشفيات|عملاء|جهات|اتصل|اتصال|تواصل/i.test(text);
}

function isExecutionRequest(text: string) {
  return /نفذ|نفّذ|سو|سوي|اعمل|أنشئ|انشئ|أضف|اضف|عدّل|عدل|غيّر|غير|احذف|رتب|جدول|موعد|مسودة|اكتب|احصر|ابحث|قارن|حلل|حفظ|احفظ|اتصل|تواصل|خطط|قسّم|قسم/i.test(text);
}

function isChatOnly(text: string, hasActions: boolean) {
  const isQuestion = /^(ليش|لماذا|كيف|وين|أين|وش|ما|ماذا|هل|اشرح|وضح|يعني|متى)/i.test(text.trim());
  return isQuestion || (hasActions && !isExecutionRequest(text));
}

function normalizeType(type: string): ActionType {
  if (type === 'calendar_event' || type === 'google_calendar') return 'calendar_file';
  if (type === 'google_sheet' || type === 'sheet' || type === 'shopping_table' || type === 'create_table') return 'csv_file';
  if (type === 'email_draft' || type === 'gmail_draft' || type === 'email_send' || type === 'send_email') return 'message_draft';

  return ['create_task', 'calendar_file', 'csv_file', 'message_draft', 'create_note'].includes(type)
    ? type as ActionType
    : 'create_task';
}
...
function splitTimes(count: number, start: string, end: string) {
  const s = timeToMinutes(start);
  const e = Math.max(timeToMinutes(end), s + count * 10);
  const step = Math.max(10, Math.floor((e - s) / Math.max(count, 1)));

  return Array.from({ length: count }, (_, i) => minutesToTime(s + i * step));
}
...
function extractTimeRange(text: string) {
  ...
  const m = source.match(
    /(?:من\s*)?(?:الساعة\s*)?(\d{1,2})(?::(\d{2}))?\s*(صباح|ص|مساء|م|عصر)?\s*(?:إلى|الى|-|لـ|ل)\s*(?:الساعة\s*)?(\d{1,2})(?::(\d{2}))?\s*(صباح|ص|مساء|م|عصر)?/i
  );

  const hour = (raw: string, mer?: string, base?: number) => {
    let h = Number(raw);
    const pm = /مساء|م|عصر/i.test(mer || '') || (!mer && base !== undefined && h <= base);
    if (pm && h < 12) h += 12;
    return h;
  };

  if (!m) return { start: '09:00', end: '10:00' };

  const s = hour(m[1], m[3]);
  const e = hour(m[4], m[6], s);

  return {
    start: minutesToTime(s * 60 + Number(m[2] || 0)),
    end: minutesToTime(Math.max(e * 60 + Number(m[5] || 0), s * 60 + 60)),
  };
}

function ensureRows(action: Action): Action {
  if (action.type !== 'csv_file') return action;

  const columns = Array.isArray(action.payload?.columns) && action.payload.columns.length
    ? action.payload.columns
    : ['البند', 'الوصف', 'الحالة', 'الموعد', 'ملاحظات'];
  ...
  return {
    ...
    payload: {
      ...
      rows: rows.length
        ...
        : [[
            action.title,
            action.description || 'تم إنشاؤه بواسطة الوكيل',
            'جديد',
            `${action.dueDate || today()} ${action.dueTime || ''}`.trim(),
            'يمكن تعديلها من نفس المحادثة',
          ]],
    },
  };
}

function toActions(raw: any, text: string, prompt: string): Action[] {
  return asArray(raw).map((a: any) =>
    ensureRows({
      id: uid(),
      type: normalizeType(a?.type || 'create_task'),
      title: a?.title || 'إجراء',
      description: a?.description || text || prompt,
      dueDate: a?.dueDate || '',
      dueTime: a?.dueTime || '',
...
function fallback(agent: Agent, prompt: string): Action[] {
  if (agent.id === 'finance') {
    return [
      {
        id: uid(),
        ...
        priority: 'high',
        payload: {
          columns: ['البند', 'المبلغ', 'الاستحقاق', 'الحالة'],
          rows: [[prompt, '', today(), 'جديد']],
        },
      },
    ];
  }
...
function summarize(actions: Action[]) {
  return asArray<Action>(actions)
    .map((a) => `${a.title} | ${a.type} | ${a.description} | ${a.dueDate || ''} ${a.dueTime || ''}`)
    .join('\n');
}

function saveDirectory(items: any[]) {
  const old = JSON.parse(localStorage.getItem(DIRECTORY_KEY) || '[]');
  ...
  localStorage.setItem(
    ...
    JSON.stringify([
      ...asArray(items).map((x) => ({
        ...x,
        id: x.id || uid(),
        source: x.source || 'Gemini',
        status: x.status || 'جديد',
        lastContact: '',
        nextFollowUp: '',
      })),
      ...old,
...
function tableText(action: Action) {
  const safe = ensureRows(action);
  const columns = safe.payload?.columns || ['العنوان', 'الوصف'];
  const rows = Array.isArray(safe.payload?.rows) ? safe.payload.rows : [[safe.title, safe.description]];

  return [columns, ...rows]
    .map((row: any[]) => asArray(row).map((cell) => String(cell ?? '')).join(' | '))
    .join('\n');
}
...
function executionReport(goal: string, done: Action[], failed: string[]) {
  const ok = asArray<Action>(done).filter((a) => a.executed);

  const count = (type: ActionType) =>
    ok.filter((a) => a.type === type).length;

  const rows = ok
    .filter((a) => a.type === 'csv_file')
    .reduce((sum, a) => sum + (Array.isArray(a.payload?.rows) ? a.payload.rows.length : 0), 0);

  const status = failed.length ? 'يحتاج مراجعة' : 'مكتمل';
  const updated = new Date().toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' });
...
export default function ExecutionAgentsPage() {
  ...
  const agent = useMemo(
    () => agents.find((x) => x.id === agentId) || agents[0],
    [agentId]
  );
  ...
  const openTasks = useMemo(
    () => memory.tasks.filter((task) => task.status !== 'done').slice(0, 5),
    [memory.tasks]
  );

  function commit(next: Session[], nextActiveId: string) {
    setSessions(next);
    saveSessions(next);
    setActiveId(nextActiveId);
  }

  function makeSession(text = ''): Session {
    return {
      id: uid(),
      agentId,
      title: text ? titleOf(text) : 'محادثة جديدة',
      icon: agent.icon,
      messages: [],
      actions: [],
      result: '',
  ...
  function deleteSession(id: string) {
    ...
    commit(next, next.find((s) => s.agentId === agentId)?.id || '');
  }

  async function executeAction(action: Action) {
    const safe = ensureRows(action);

    if (safe.type === 'create_task') {
      await memory.saveTask({
        title: safe.title,
        description: safe.description,
        priority: safe.priority || 'medium',
        status: 'todo',
        dueDate: safe.dueDate || today(),
        dueTime: safe.dueTime || '',
        listName: agent.name,
        recurrence: 'none',
      });
    }

    if (safe.type === 'create_note') {
    ...
    if (safe.type === 'calendar_file') {
      ...
      try {
        ...
        await memory.saveNote({
          title: `موعد محفوظ: ${safe.title}`,
          body: `${safe.description}\n${safe.dueDate || today()} ${safe.dueTime || '09:00'}`,
          tags: ['calendar', 'agent', agent.id],
          pinned: true,
        });
      }
    }

    if (safe.type === 'message_draft') {
      try {
        await createGmailDraft({
          to: safe.payload?.to || '',
          subject: safe.payload?.subject || safe.title,
          body: safe.payload?.body || safe.description,
        });
      } catch {}

      await memory.saveNote({
        title: `مسودة Gmail: ${safe.title}`,
        body: `تم حفظ المسودة.
...
${safe.payload?.body || safe.description}`,
    ...
    if (safe.type === 'csv_file') {
      const table = upsertTable({
        title: safe.title,
        columns: safe.payload?.columns || ['العنوان', 'الوصف'],
        rows: safe.payload?.rows || [[safe.title, safe.description]],
        source: agent.name,
      ...
      try {
        ...
        await memory.saveNote({
          title: `جدول محفوظ داخل التطبيق: ${table.title}`,
          body: `${tableText(safe)}

افتح /tables لمشاهدة الجدول وتصديره يدويًا.`,
          tags: ['table', 'agent', agent.id],
          pinned: true,
        });
      }
    }
  ...
  async function runActions(session: Session, goal: string, summary: string, actions: Action[]) {
    ...
    for (const action of asArray<Action>(actions).map(ensureRows)) {
      try {
        await executeAction(action);
        done.push({ ...action, executed: true });
      } catch {
        done.push({ ...action, failed: true });
        failures.push(action.title);
      }
    }
...
${executionReport(goal, done, failures)}
`;
    ...
    const updated: Session = {
      ...
      messages: [
        ...session.messages,
        { id: uid(), role: 'agent', text: reply, at: now() },
      ],
      actions: done,
      result: summary,
      updatedAt: now(),
      title: session.title === 'محادثة جديدة' ? titleOf(goal) : session.title,
    };
    ...
    addExecutionLog({
      ...
      goal,
      status: failures.length ? 'needs_user' : 'done',
      summary,
      results: done.filter((a) => a.executed).map((a) => `${a.title} (${a.type})`),
      failures,
      nextSteps: failures.length
        ? ['راجع الإجراء الفاشل أو أعد المحاولة من نفس المحادثة.']
        : ['راجع النتائج في المهام/التقويم/الدليل/الجداول/سجل التنفيذ.'],
    });
  }
  ...
  async function directoryPlan(text: string) {
    ...
    const res = await fetch('/api/directory/gemini-search', {
      method: 'POST',
    ...
    const contacts = asArray(data.contacts).slice(0, 8);
    ...
    const rows = contacts.length
      ? contacts.map((c: any, i: number) => [
          ...
          times[i],
          'لم يتم الاتصال',
          c.notes || 'تحقق قبل الاتصال',
        ])
      : [[text, '', '', '', '', '', range.start, 'يحتاج بحث إضافي', 'لم يرجع Gemini نتائج كافية']];

    return {
      summary: `تم حفظ ${contacts.length} جهة في الدليل وإنشاء جدول ومواعيد متابعة من ${range.start} إلى ${range.end}.`,
      actions: [
        {
          id: uid(),
          type: 'csv_file' as ActionType,
          title: 'جدول جهات الاتصال',
          description: 'جدول متابعة الاتصال والنتائج.',
          dueDate: today(),
          priority: 'high' as Priority,
          payload: {
            columns: ['الجهة', 'الهاتف', 'البريد', 'الموقع', 'العنوان', 'الخريطة', 'وقت الاتصال', 'الحالة', 'ملاحظات'],
            rows,
          },
        },
        ...contacts.map((c: any, i: number) => ({
          id: uid(),
          type: 'calendar_file' as ActionType,
          title: `اتصل بـ ${c.name}`,
          description: `الهاتف: ${c.phone || 'غير متوفر'}\nالعنوان: ${c.address || '-'}\nالموقع: ${c.website || c.mapsUrl || '-'}`,
          dueDate: today(),
          dueTime: times[i],
          priority: 'high' as Priority,
        })),
  ...
  async function chatReply(session: Session, text: string) {
    ...
    const conversation = session.messages
      .map((m) => `${m.role === 'user' ? 'المستخدم' : 'الوكيل'}: ${m.text}`)
      .join('\n');

    const r = await fetch('/api/agents/personal', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        agentId: 'assistant',
        prompt: `${conversation}${previous}
...
هذه رسالة محادثة فقط وليست أمر تنفيذ. أجب كموظف يشرح ما حصل وما مكان النتيجة وما الخطوة المناسبة. لا تنشئ actions.`,
  ...
  async function send() {
    ...
    const sessionWithUser: Session = {
      ...base,
      title: base.messages.length ? base.title : titleOf(text),
      messages: [
        ...base.messages,
        { id: uid(), role: 'user', text, at: now() },
      ],
      updatedAt: now(),
    };
    ...
    try {
      ...
      const conversation = sessionWithUser.messages
        .map((m) => `${m.role === 'user' ? 'المستخدم' : 'الوكيل'}: ${m.text}`)
        .join('\n');

      const instruction = `${conversation}${previous}

نفّذ فقط إذا كان الطلب الحالي فيه أمر واضح.
إذا كان المستخدم يعدّل نتيجة سابقة فعدّلها.
لا تنشئ جدولًا فارغًا.
لا تنزّل ملفات.
الجداول يجب أن تحفظ داخل التطبيق.`;

      if (agent.id === 'style' && image) {
        const imageBase64 = await fileToDataUrl(image);

        const r = await fetch('/api/agents/vision-style', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            prompt: instruction,
        ...
        plan = {
          summary: data.text || 'تم تحليل الصورة وتنفيذ المطلوب.',
          actions: toActions(data.plan?.actions, data.text || '', text),
        };
...
أنت ${agent.name}.
أرجع JSON actions مع summary/guidance عند وجود تنفيذ حقيقي.
إن لم يوجد تنفيذ، أجب فقط ولا ترجع actions.`,
          }),
        });

        const data = await r.json();
        const msg = data.plan?.guidance || data.plan?.summary || data.text || 'تمت مراجعة الطلب.';

        plan = {
          summary: msg,
          actions: toActions(data.plan?.actions, msg, text),
        };
      }
      ...
      if (!plan.actions.length) {
        upsertSession({
          ...
          messages: [
            ...sessionWithUser.messages,
            { id: uid(), role: 'agent', text: plan.summary, at: now() },
          ],
          result: plan.summary,
          updatedAt: now(),
        });
      } else {
        await runActions(sessionWithUser, text, plan.summary, plan.actions);
      }
      ...
      upsertSession({
        ...
        messages: [
          ...sessionWithUser.messages,
          { id: uid(), role: 'agent', text: `فشل التنفيذ: ${msg}`, at: now() },
        ],
        updatedAt: now(),
      });
    } finally {
  ...
  return (
    <main className="agents-shell" dir="rtl">
      ...
      <header className="hero-card">
        ...
        <a className="hub" href="/execution">سجل التنفيذ</a>
        <span className="eyebrow">Agent Workspaces V2</span>
        <h1>الوكلاء التنفيذيون</h1>
        <p>الأوامر تنفذ. الأسئلة تظل محادثة فقط. الجداول محفوظة داخل التطبيق.</p>
      </header>
      ...
      <section className="agent-grid">
        {agents.map((a) => (
          <button
            key={a.id}
            className={`agent-tile ${a.id === agentId ? 'active' : ''}`}
            onClick={() => {
              setAgentId(a.id);
              const first = sessions.find((s) => s.agentId === a.id);
              setActiveId(first?.id || '');
            }}
          >
            <b>{a.icon}</b>
            <strong>{a.name}</strong>
            <small>{a.promise}</small>
          </button>
        ))}
      </section>
      ...
      <section className="layout">
        <aside className="sessions-panel">
          <div className="section-title">
            <h2>المحادثات</h2>
            <button onClick={() => upsertSession(makeSession())}>+</button>
          </div>

          {currentAgentSessions.map((s) => (
            <div
              key={s.id}
            ...
            >
              ...
              <div>
                <b>{s.title}</b>
                <small>{new Date(s.updatedAt).toLocaleString('ar-SA')}</small>
              </div>
              <button
                onClick={(e) => {
                  e.stopPropagation();
          ...
          {!currentAgentSessions.length && (
            <small>لا توجد محادثات لهذا الوكيل. اكتب طلبك وسيتم إنشاء محادثة تلقائيًا.</small>
          )}
        </aside>
        ...
        <section className="workspace">
          <div className="panel">
            ...
            <textarea
              ...
              placeholder="اكتب أمر تنفيذ، أو سؤال متابعة مثل: ليش سويت كذا؟"
            />

            <button className="primary" disabled={busy} onClick={send}>
              {busy ? 'جاري...' : active ? 'إرسال' : 'أنشئ محادثة وأرسل'}
            </button>
          ...
          <div className="panel">
            ...
            <div className="chat-box">
              ...
              {!active && <small>لا توجد محادثة مفتوحة.</small>}
            </div>
          </div>
        </section>
      </section>
      ...
      <section className="panel">
        ...
        <div className="action-list">
          {active?.actions.map((a) => (
            <article
              key={a.id}
              className={a.failed ? 'failed action-card' : a.executed ? 'done action-card' : 'action-card'}
            >
              <div>
                <b>{a.title}</b>
                <p>{a.description}</p>
                <small>
                  {a.type} · {a.dueDate || 'بدون تاريخ'} · {a.dueTime || 'بدون وقت'}
                  {a.type === 'csv_file' ? ` · صفوف: ${a.payload?.rows?.length || 0}` : ''}
                </small>
              </div>
              <button disabled>{a.failed ? 'فشل' : 'تم'}</button>
            </article>
          ))}
          {!active?.actions.length && <small>لا توجد إجراءات تنفيذية في هذه المحادثة.</small>}
        </div>
      </section>

      <section className="panel">
        <h2>مهام مفتوحة من النظام</h2>
        <div className="task-list">
          {openTasks.map((t) => (
            <div key={t.id} className="task-row">
              <b>{t.title}</b>
              <small>{t.dueDate || 'بدون تاريخ'} · {t.priority}</small>
            </div>
          ))}
        </div>
      </section>
